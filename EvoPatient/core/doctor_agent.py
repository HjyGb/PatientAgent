"""Doctor agent: simulates a doctor's behavior in medical dialogue."""
...
from core.agent_evolve import store_doctor_qa, agent_evolving_doctor
...
class Doctor:
    ...
    def doctor_qus(self, answer, patient_score, rel, faith, human):
        ...
        if not evolve_csv.is_file():
            with evolve_csv.open("w", newline="", encoding="utf-8") as file:
        ...
        if doctor_evolve_info:
            for idx, info_dic in enumerate(doctor_evolve_info, start=1):
                ...
                few_shot_example += f"Example {idx}: Question: {doctor_qus} Patient answer: {patient_answer} New question: {new_qus}"
        else:
        ...
        if new_office:
            ...
            for office in offices:
                if check_files(doctor_record_dir, office):
                    print(f"Doctor for {office} already exists, skipping recruitment")
                    continue
                ...
                print(f"Recruited {office} doctor")
        return new_office
    ...
    def doctor_crisis_answer(self, office, patient_crisis):
        ...
        if auto:
            ...
            doctor_answer = input("Enter reply: ")
        return doctor_answer


def check_files(directory: Path, prefix: str) -> bool:
    """Check if any file in the directory starts with the given prefix."""
...
def overall_assessment_doctor(question, useful_info, answer):
    from core.overall_assessment import overall_assessment_doctor as _assess
    return _assess(question, useful_info, answer)
