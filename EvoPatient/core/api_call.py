...
def llm_api_stream(messages, model: str | None = None, temperature: float = 0.2):
    """Streaming chat completion — yields content tokens, StopIteration.value = full text.
    ...
    Usage:
        ...
        # full contains complete response
    """
    model = model or LLM_MODEL
    response = client.chat.completions.create(
        messages=messages,
    ...
    for chunk in response:
        ...
        if hasattr(chunk, "usage") and chunk.usage:
            token_counter(chunk.usage.model_dump())
    return full_text
...
def _get_local_embedding_model():
    ...
    if _LOCAL_EMBEDDING is None:
        ...
        # Support both old (get_sentence_embedding_dimension) and new (get_embedding_dimension) API
        if hasattr(_LOCAL_EMBEDDING, 'get_embedding_dimension'):
            dim = _LOCAL_EMBEDDING.get_embedding_dimension()
        else:
